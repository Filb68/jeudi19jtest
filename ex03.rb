...
nbjournalists = journalistes.length

puts "le  nombre de journalistes est : #{nbjournalists} "

#Valeurs de la boucle faisant défiler chaque journaliste pour le traitement
i = 0
num = nbjournalists
...
#Boucle faisant défiler chaque journalistes
while i < num  do
    jrnlist = journalistes[i]
    #localiser le 2nd caractère 
     compar = jrnlist[1]  
    #localiser le 2nd caractère 


    #Traitement : le second caractère est-il une Majuscule?
        #compteur = 0
            if
            compar =~ /[A-Z]/
...
puts (" les Handles commençant par une Maj sont au nombre de #{compteur}" ) 